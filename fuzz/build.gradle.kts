...
val targetedReplaySelfCheckDir = layout.buildDirectory.dir("targeted-replay-self-check")
...
val targetedReplaySelfCheckTargets =
    fuzzCampaignTargets.filter { target ->
        target.taskName == "fuzzIdentityReference" ||
            target.taskName == "fuzzNip55SignerProtocol"
    }

val syncTargetedReplaySelfCheck =
    tasks.register<Sync>("syncTargetedReplaySelfCheck") {
        from(layout.projectDirectory.dir("src/test/resources")) {
            exclude("dev/ipf/whitenoise/android/fuzz/*FuzzTestInputs/**")
        }
        targetedReplaySelfCheckTargets.forEach { target ->
            val classSimpleName = target.testClass.substringAfterLast('.')
            val destination =
                "dev/ipf/whitenoise/android/fuzz/" +
                    "${classSimpleName}Inputs/${target.methodName}"
        ...
        into(targetedReplaySelfCheckDir)
    }
...
fun Test.configureFuzzReplay(
    replayCorpusOutput: File,
    targetedReplay: Boolean,
) {
...
tasks.register<Test>("replayFuzzRegression") {
    ...
    configureFuzzReplay(
        replayCorpusOutput = syncFuzzReplayCorpus.get().destinationDir,
        targetedReplay = project.findProperty("fuzzReplayInputsDir") != null,
    )
}

val replayTargetedFuzzRegressionSelfCheck =
    tasks.register<Test>("replayTargetedFuzzRegressionSelfCheck") {
        group = "verification"
        description = "Replay passing Identity and NIP-55 corpora through the targeted path"
        dependsOn(tasks.testClasses, syncTargetedReplaySelfCheck)
        configureFuzzReplay(
            replayCorpusOutput = syncTargetedReplaySelfCheck.get().destinationDir,
            targetedReplay = true,
        )
        filter {
            targetedReplaySelfCheckTargets.forEach { target ->
                includeTestsMatching(target.testClass)
            }
        }
    }
...
tasks.register("replayAllFuzzRegression") {
    ...
    dependsOn(
        ...
        replayTargetedFuzzRegressionSelfCheck,
