...
The probe creates a temporary identity and private group, uploads one generated
approximately 64 KiB PNG to the configured default media service, and verifies every
download against those generated bytes. It reads no personal media or accounts.
...
The network phase makes 20 sequential native calls with no Android plaintext
cache, then one 16-request batch through the actual three-slot gate. All calls
reuse the same reference; transport and server caches may already be warm.
The batch reports each request's completion latency including queue wait, not
16 independent album trials. Local phases use 20 new encrypted-cache entries
and the production decoder. Report first-sample effects through the maximum
as well as nearest-rank p50/p95.

This probe does not measure receipt-to-visible pixels, distinct-image albums,
stalled-locator failover, internal network/crypto phases, or a signed release
