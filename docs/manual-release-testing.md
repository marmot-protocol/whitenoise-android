...
15. [ ] **DIC-002 — Dictation settings/results and background controls** — Exercise each displayed dictation mode/provider/language option plus cold first-tap, provider-disconnect/permission recovery, finish/cancel/error paths, and at least two minutes of manual silence followed by actual speech. During capture, navigate Home and another chat, then use each persistent or notification action (Cancel, Paste, and Send) before returning to the origin conversation. → **Expected:** In-app mode stays on native controls without automatically opening the provider Activity, recovery is bounded and remains cancellable, silence does not end manual dictation, selection persists, each action changes only the immutable origin draft/chat, any unrelated current conversation remains unchanged, reopening the origin shows the intended result, Cancel preserves the prior draft, and failures provide recovery without sending text.
