...
11. [ ] **ACC-011 — Encrypted backup create** — Create an encrypted backup with a test password, test mismatch/weak or empty input, and save through Android. → **Expected:** Invalid input is blocked; valid export produces one nonempty file only after explicit destination choice.
12. [ ] **ACC-012 — Encrypted backup restore** — On a clean disposable install restore the backup with wrong then correct password. → **Expected:** Wrong password imports nothing and gives a safe error; correct password restores only the expected identity/state.
