...
/** Uses the standard-width treatment only for a rendered caption paired with portrait media. */
internal fun shouldExpandCaptionedPortrait(
    hasCaption: Boolean,
    dim: String?,
): Boolean = hasCaption && aspectRatioFromDim(dim)?.let { it < 1f } == true
