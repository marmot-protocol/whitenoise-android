...
object AmberSignerRelay {
    ...
    private val handledPackageByRequestId = ConcurrentHashMap<String, String>()
    ...
    internal fun recordHandledSignerPackage(
        ...
        if (!requestId.isNullOrBlank() && !packageName.isNullOrBlank()) {
            handledPackageByRequestId[requestId] = packageName
        }
    }

    internal fun consumeHandledSignerPackage(requestId: String?): String? = requestId?.let(handledPackageByRequestId::remove)
}
