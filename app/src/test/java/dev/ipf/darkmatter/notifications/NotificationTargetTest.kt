...
class NotificationTargetTest {
    ...
    fun fromUpdate_invite_dropsMessageId() {
        val target = NotificationNavigation.fromUpdate(
    ...
    fun parseExtras_validMessage_roundTrips() {
        val target = NotificationNavigation.parseExtras(
    ...
    fun parseExtras_inviteIgnoresMessageId() {
        val target = NotificationNavigation.parseExtras(
            NotificationNavigation.ACTION_OPEN, "a", "g", "m1", "INVITE",
        )
    ...
    ): NotificationUpdateFfi = NotificationUpdateFfi(
